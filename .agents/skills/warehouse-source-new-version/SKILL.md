...
- ShipStation v1→v2 is a host+auth+pagination+date-format swap (ssapi/basic-auth/Pacific → api.shipstation.com/v2/API-Key-header/ISO-8601-UTC), but `get_schemas` and `SourceConfig.fields` are version-blind (no `api_version` param; version is a column, not in `job_inputs`), so per-version schema/credential-field divergence can't be expressed without a shared-layer change.
- Do NOT advertise (`supported_versions`) or default to a version whose resource set / schema surface diverges from what the shared `get_schemas` returns: new sources are stamped `default_version` and then discover schemas version-blind, so they'd sync the old version's endpoints against the new host and 404 on every table while `validate_credentials` still passes (false green). Land the transport groundwork but keep the new version out of `supported_versions`/`default_version` (reachable only by an explicit pin) until schemas can diverge. A per-version credential probe path (e.g. `/stores` vs `/carriers`) is itself the tell that the resource sets differ.
...
- New auth scheme = re-check redirect safety: `requests` only strips the `Authorization` header on a cross-host redirect, not custom headers, so a header-based API key (e.g. v2 `API-Key`) leaks across a 3xx. Build that session with `make_tracked_session(..., allow_redirects=False)`; a basic-auth (v1) session is already safe.

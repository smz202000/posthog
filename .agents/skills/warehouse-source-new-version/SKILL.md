...
- (seed) Stripe: response shapes differ enough across date versions that canonical column hints must be gated per version; newer versions auto-infer schema instead. **Superseded — do not gate write hints alone, see below.**
- CAUTION for `has_managed_hogql_schema=True` sources (Stripe): the canonical `external_table_definitions` drive BOTH the write-side `column_hints` and the read-side HogQL fields (`WarehouseTable.hogql_definition`), and neither `get_dlt_mapping_for_external_table` nor `external_tables` is api_version-aware. Gating only the write hints per version (e.g. `column_hints=None` for the new version) desyncs write from read: the new version's physical delta columns get auto-inferred but are still queried through the old canonical mapping (which renames raw columns, e.g. `customer`→`customer_id`, `created`→`created_at`). Keep the SAME hints for every version so write and read stay consistent. Real per-version schema divergence needs the READ path versioned too (version-aware `external_table_definitions` + a gated `hogql_definition`), not a write-side-only gate — and that needs real data from the new version to author, so verify against a live sync, not just unit tests.
- Stripe: dispatch was already wired — `source_for_pipeline` threads `resolve_api_version(inputs.api_version)` → `stripe_source` → `get_rows` → `StripeClient(stripe_version=...)`. A stable-version add is just the new label in `supported_versions` + the default flip; the request goes out under the new version while the managed schema stays the shared canonical one. No per-version modules.
- Stripe webhook management (`create/delete/update/get_external_webhook_info`) builds its own `StripeClient` at source-setup time and stays hardcoded to acacia — the per-instance pin isn't reachable there and endpoint CRUD responses are version-stable, so don't thread the sync pin into it (would touch the shared `WebhookSource` ABC). If a future version reshapes webhook _event payloads_ the source reads, that becomes a real gap worth revisiting.

...
- Kustomer: DON'T assume the version label is a URL path segment. Kustomer exposes v1.0 and v2 doc toggles, but the core list resources (customers, conversations, users, teams, tags, brands) are served under `/v1/` for BOTH — the "v2" toggle keeps them at `/v1/`. Building `/v2/<resource>` 404s. So v2 support here is a declaration-only change: `supported_versions=("v1","v2")` + `default_version="v2"`, with the request layer untouched (every version resolves to the same `/v1/` requests). The pin is still recorded/exposed by the framework without any request-layer branch. Verify actual served paths against the vendor reference before threading a version into the URL.

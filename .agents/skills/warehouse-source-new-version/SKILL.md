...
- HubSpot: date versions ("2026-03") are a URL path segment that replaces the legacy `/crm/v3/` AND `/crm/v4/` segments (date versions subsume both); a single `apply_crm_api_version(path, version)` helper that returns the path unchanged for the legacy `v3` pin keeps existing syncs byte-for-byte while rewriting only for date pins. Thread the resolved version down to every place a CRM URL is built (objects, `/search`, `/properties`, v4 association batch-read).

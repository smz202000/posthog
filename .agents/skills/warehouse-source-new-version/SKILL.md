...
- Stripe: dispatch was already wired — `source_for_pipeline` threads `resolve_api_version(inputs.api_version)` → `stripe_source` → `get_rows` → `StripeClient(stripe_version=...)`. A new version is just adding the label to `supported_versions` + gating `column_hints` on a `HINT_COMPATIBLE_VERSIONS` set (acacia only; `column_hints=None` → auto-infer). No per-version modules.
- Stripe (any source with `has_managed_hogql_schema=True`): the version gate lives on `column_hints`, not on the request — feed hints only for the versions they were authored against, `None` otherwise; existing pinned sources stay byte-for-byte because their version keeps its hints.

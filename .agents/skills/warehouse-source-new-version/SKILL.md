...
- HubSpot: date versions ("2026-03") do NOT replace the `/crm/v3/`/`/crm/v4/` segment in place — the version segment moves to _after_ the API resource name, so `/crm/v3/objects/contacts` → `/crm/objects/2026-03/contacts` and `/crm/v4/associations/a/b/batch/read` → `/crm/associations/2026-03/a/b/batch/read` (verify each path against the vendor's date-versioned API reference, don't extrapolate). A single `apply_crm_api_version(path, version)` helper returns the path unchanged for the legacy `v3` pin (existing syncs byte-for-byte) and re-inserts the date segment after the resource name for date pins. Thread the resolved version down to every place a CRM URL is built (objects, `/search`, `/properties`, v4 association batch-read).
- Version-independent auth endpoints (HubSpot `/oauth/v1/…` token/validation calls) are NOT under the CRM version scheme — leave them alone on a CRM version bump.
- Give the threaded `api_version` param a default of the legacy label on internal request helpers so existing direct-call tests stay byte-for-byte; resolve the real pin only once, at `source_for_pipeline` via `self.resolve_api_version(inputs.api_version)`.

...
- Shopify: version is the `{version}` URL segment of the Admin GraphQL endpoint (`/admin/api/{version}/graphql.json`) — the source had NO dispatch, just a hardcoded `SHOPIFY_API_VERSION` constant. Minimal add: thread `resolve_api_version(inputs.api_version)` from `source_for_pipeline` → `shopify_source(api_version=...)` → the URL `.format`. Existing pins backfilled by migration 0075, so flipping `default_version` only affects newly created sources (byte-for-byte safe, no migration for a plain update).
- Shopify: the token/permission probes (`_authenticated_session`, used by `validate_credentials`/`check_endpoint_permissions`) run pre-pin, so default their `api_version` arg to the new version — the `{ shop { id } }` and permission queries are version-agnostic, so no behavior change vs the old default.
- Test both the resolution path (`source_for_pipeline` with `shopify_source` mocked → assert the `api_version` kwarg per pin incl. `None`→default) AND the request path (`shopify_source` with a fake session → assert the version is the posted URL segment); one guards the default flip + threading, the other guards the URL build.

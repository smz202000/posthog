...
- Formerly-unversioned sources are NOT NULL-pinned: migration `0075_backfill_externaldatasource_api_version` backfilled every source type absent from its snapshot map to `DEFAULT_API_VERSION="v1"`, and creation stamps the class `default_version`. So existing rows already carry a literal `"v1"` pin. Giving `"v1"` real (differing) endpoint meaning silently moves those rows — either repin them `"v1"`→new in a written-not-run migration (the skill's migration rule), or keep the diverging resources pinned to the version existing rows actually sync from so `"v1"` and the new default resolve identically (what incident.io did — the per-version map is just the seam for a later, migration-backed bump).

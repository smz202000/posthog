...
- Google Ads: decode responses against the version they were produced under — make the protobuf type-URL resolver dispatch on the URL's version segment rather than a hardcoded label; enum data-type *constants* are stable across versions so they can stay on one version's module.

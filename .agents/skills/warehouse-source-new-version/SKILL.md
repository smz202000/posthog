...
- Google Ads: the version is baked into the pinned `google-ads` SDK's generated `vNN.*` modules, so a version add needs a package bump to a release bundling old+new (v24 first shipped in `google-ads==30.1.0`, v23 retained); pin the version on every `get_service(..., version=...)` call because the SDK's client default is the newest bundled version, so on bump an unpinned call silently moves existing sources forward.
- Google Ads: decode responses against the version they were produced under — make the protobuf type-URL resolver dispatch on the URL's version segment rather than a hardcoded label; enum data-type _constants_ are stable across versions so they can stay on one version's module.
- Multi-version sources must also thread the pin into schema **discovery**, not just sync: the periodic `sync_new_schemas` activity calls `get_schemas` and passes `source.api_version` only when `len(supported_versions) > 1`, so a multi-version source's `get_schemas` override must accept `api_version` and resolve it via `self.resolve_api_version(...)` instead of hardcoding `self.default_version`.

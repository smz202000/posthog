...
- Brex: versions each product API independently in the URL path (transactions/users on `/v2`, expenses/vendors on `/v1`) with no global version header or segment — a "v2" bump does NOT mean rewriting every path to `/v2/`; `/v2/expenses` and `/v2/vendors` do not exist, so blindly bumping them would 404. The source already targets each API's current path, so v1 and v2 resolve to identical requests today; declare both, default to v2, and add a consumed per-version path override (`versioned_paths` dict, resolved through a helper) as the seam for the day a specific API genuinely ships a new path. Test both versions produce the current paths.
- When a source has no version declaration yet, existing rows are NULL-pinned and resolve to `default_version`; flipping the default to the new label silently moves them onto it — so the new default MUST produce byte-for-byte identical requests, or you've repinned customers without a migration.

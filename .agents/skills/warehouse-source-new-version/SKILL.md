...
- Zonka Feedback: vendor versions its REST API via the account's region subdomain, not a URL path segment or version header, so v1 and v2.1 issue byte-for-byte identical requests (same host, same paths `/responses` `/surveys` `/contacts`, no version header). With no request divergence, the version bump is a pure declaration + default flip — declare `supported_versions`/`default_version` and stop. Do NOT thread the resolved version into the request layer or key the host on version: with every version building the same request that scaffolding is inert (and `base_url` also feeds `check_access`/`get_non_retryable_errors`, so a real future divergence isn't a one-line change anyway). New sources are stamped `default_version` at creation, so flipping the default is safe precisely because both versions build the same request.

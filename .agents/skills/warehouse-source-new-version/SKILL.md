...
- Zonka Feedback: vendor versions its REST API via the account's region subdomain, not a URL path segment or version header, so v1 and v2.1 issue identical requests — the version bump is a pure declaration + default flip with the resolved version threaded to the URL builder (v1→v2.1 both map to the same host template, keyed by version for a future divergence). New sources are stamped `default_version` at creation (`_create_external_data_source`), so a source built under the unversioned default already carries that label as a pin — flipping the default is byte-for-byte safe only because both versions build the same request. Fall back to the default host template for an undeclared pin rather than raising, to mirror `resolve_api_version`'s honor-the-pin stance.
- Test gotcha: `parameterized.expand` can't co-exist with pytest's `monkeypatch` fixture on the same method (positional-arg collision) — use `with pytest.MonkeyPatch.context() as mp:` inside the test instead.

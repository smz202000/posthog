...
@shared_task(**EMAIL_TASK_KWARGS)
@skip_team_scope_audit
def send_project_secret_api_key_rolled(
    ...
    message = EmailMessage(
        use_http=True,
        campaign_key=f"project-secret-api-key-rolled-{team.uuid}-{timezone.now().timestamp()}",
        subject=f"Project secret API key has been deactivated for {team.name}",
        template_name="project_secret_api_key_rolled",
        template_context={

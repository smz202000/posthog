...
from posthog.tasks.email import (
    ...
    send_project_secret_api_key_rolled,
...
class TestEmail(APIBaseTest, ClickhouseTestMixin):
    ...
    def test_send_project_secret_api_key_rolled(self, MockEmailMessage: MagicMock) -> None:
        mocked_email_messages = mock_email_messages(MockEmailMessage)
        ...
        send_project_secret_api_key_rolled(self.team.id, key.id, "phs_...abcd", "This key was detected by GitHub.")

        assert len(mocked_email_messages) == 1
        message = mocked_email_messages[0]
        assert message.send.call_count == 1
        assert message.template_name == "project_secret_api_key_rolled"
    ...
    def test_send_project_secret_api_key_rolled_respects_opt_out(self, MockEmailMessage: MagicMock) -> None:
        mocked_email_messages = mock_email_messages(MockEmailMessage)
        ...
        send_project_secret_api_key_rolled(self.team.id, key.id, "phs_...abcd", "")
